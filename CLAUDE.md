...
### 关键文件
```
main.go               启动：include.Context → 解析 config → box.New → Start（镜像官方 CLI 写法，跟随其 config schema）
configs/config.json   sing-box 配置：mixed 入站 + route sniff + 域名 reject 示例 + service/api(+dashboard)
go.mod                require sing-box + replace 到子模块
third_party/sing-box  【上游子模块】sing-box 源码（testing 分支）
webui/                【上游 vendored 副本】官方 dashboard，我们自己维护/改造
```
...
- **里程碑 1（🟡 进行中）** ✅白名单默认拒绝 + ✅`AppendTracker` 检测器遥测 stub（`detector.go`）+ ✅开启 Clash API。**待做**：自建 Go `/api` + 自动处置闭环（检测异常 → 断连）。
- **里程碑 2** 自建 React 应用（管理+安全）：先做**节点/订阅管理**（订阅解析→生成 outbound→热重载，逻辑借鉴 s-ui）+ 连接/告警视图。
