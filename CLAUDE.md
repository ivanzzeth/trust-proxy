...
internal/proxygroups/      代理分组（Config{AutoCountry,ExcludeCountries,Groups,**Failover**}；Failover=每个 urltest 组的切换积极度,默认粘性:tolerance 150ms + **不中断已有连接**,见「登录到一半突然断连」那条坑）+ 国家解析(country.go：旗emoji/中英/国码→ISO)；injectOutbounds 据此建 Auto(urltest全部)+**🌏 Overseas 共享组**(urltest,成员=国家∉ExcludeCountries 的节点;**仅当排除真的去掉≥1节点才建**,否则 Auto 已安全、指向它的规则 self-heal 回 Auto)+按国家 urltest 组+用户组(select/urltest,filter country/regex/manual)，proxy 改 selector(default Auto)。**Overseas 组**是「地区受限服务 failover」的载体:Anthropic/OpenAI 拒 HK/CN,geofenced 包(Claude/OpenAI/Cursor)走 Overseas→在允许地区间自动切、绝不落被封地区。ExcludeCountries 默认 HK/MO/CN(`DefaultExcludeCountries`,旧 store 加载时一次性迁移;nil=未设→填默认、非nil空=不排除),Proxies 页可改。sing-box 只有 selector/urltest,无 load-balance
internal/proxyscore/       **节点打分**（真实流量,非 generate_204 探测）：`score = (w_rel×可靠性 + w_lat×延迟 + w_tp×吞吐)/Σw`,观测存 data/proxyscores.json、超 `stale_hours`(默认6h) 的条目加载时丢弃(换订阅/换网络后旧数字描述的是另一条链路)。三条支柱各对应一次真实事故：① **预热**——攒满 `min_samples`(默认10) 真实样本前恒为中性 100，冷启动一次倒霉拨号不该把全组降级；② **连击而非均值**——第 N 次连续失败扣 N 倍(封顶 `max_streak`)，均值会忘记「它已经死了十分钟」，连击不会而且可解释(UI 直接印连击数)；③ **断路器(failsafe-go)与分数正交且绝不排除**——分数答「它一直以来有多好」(要 10 个样本)，断路器答「它现在还能不能用」(**从第 1 个样本就工作**,这才是死节点没法顶着预热 100 被选中的原因)。④ **黑洞判定**——握手通、发出去了、回包恒 0 的成员直接 0 分（`blackhole_streak` 默认 3，-1=关），**且压过预热**：假节点最能害人的正是它的头十条连接。⑤ **长流卡死（stream stall）**——打分只影响*新*拨号，挂死的长连接（大上传后下行静默，如 Cursor Agent）要靠 `wrapStreamStall` 中途掐断并 `RecordStreamStall` 降级（默认静默 20s / 武装 64KiB / 最短存活 8s；`stream_stall_sec=-1` 关）。断路器 open **只降到最后备选**，见下「只降级不硬排除」。配置走 `proxygroups.Config.Scoring`(同一主题+复用 profile/posture 快照管道)，观测是观测、绝不被 profile 快照带走。本包**不 import sing-box**，桥接在 `internal/gateway/scorer.go` + `streamstall.go`
