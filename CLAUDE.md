...
`install|uninstall`(**本地 root**;`service install|uninstall` 是隐藏别名,老笔记还能用) | `env`(机器事实源+`action`) | `service status` | `status` | `auth bootstrap|login|ticket|whoami|state|register|registration` | `user`/`apikey`/`request` | `acl ls|add|rm <permit|deny|no-proxy>`（`add --note` 备注；`ls` 在条目旁打印 `# note`；备注存 `notes` map，键=`<dim>:<value>`，数据面不读）| `rules ls`(生效视图) `rules custom|packs|sets …`（`custom move <id> top` 置顶非 pin）| `dns get|set`（`--direct-server` 等单项 patch，`-f` 整档替换）| `mode get|set|confirm`（`--guard` 死亡开关）| `routing get|set`(Rule/Global) | `posture get|set` | `final get|set` | `profile ls|save|activate|rm` | `proxies ls|select|delay|scores|scores-reset|nodes|disable|enable`（`scores` 打印公式表头 + 每节点分项，预热中显示 `100 (warming 4/10)` 而不是光秃秃的 100；黑洞节点显示 `BLACKHOLE (N dead connections…)` 而不是一个很低的分数；`nodes` 列 live/disabled/junk；`disable` 按 tag 持久踢出 Auto，刷新订阅仍生效）| `groups get|set|failover|scoring`（`failover`/`scoring` 无参数=查看，单项 patch：`--tolerance/--interrupt/--probe-interval/--idle-timeout`、`--min-samples/--weight-*/--reward/--penalty/--max-streak/--latency-*/--throughput-good/--tie-margin/--breaker-*/--stale-hours/--blackhole-streak/--stream-stall/--stream-stall-min-upload/--stream-stall-min-age/--disabled`）| `endpoints ls|add|toggle|rm` | `tun get|set`（含 `--exclude-package/--include-package`）| `inbound get|set|confirm`（**代理入站监听点**，`--listen/--port`，`--guard` 死亡开关；非 loopback 且无代理密码会被拒）| `retention get|set`（日志/历史轮转，无参数=查看，「未设置」显示实际生效的默认值——与 `groups failover` 同形）| `defaults`（**服务端的默认值总表**，UI 的「恢复默认」与「(默认 X)」注解都从它取）| `detect get|set|fingerprints` | `autoblock on|off` | `detections ls|stats` | `history ls|stats` | `node ls|add|rm`(fleet) | `sub add|import|ls|apply|unapply|rm|refresh`（apply 可叠加多订阅合并进 proxy 组） | `conn ls|kill`(底层 Clash 原语→:21586) | `proxy gen|run|stop`(**本地离线**,不经 SDK——出口机上没有网关在跑；`--json` 与 `POST /api/proxy-gen` 同形)。
...
internal/quarantine/       **网关自行封禁**的目的地（data/quarantine.json，与 posture 无关）；`injectQuarantine` 与黑名单同在 L1 地板，但独立存储——切 Strict/Split 或激活 Profile 会整份替换 deny 列表，防御性封禁不能跟着消失；`POST /api/quarantine/permit` = 解禁+加白（误伤运维设施的常规恢复）
...
internal/blacklist/        出网黑名单（域名/关键字/正则/IP → reject，injectBlacklist 注入在 sniff 之后、白名单之前；可选 `notes` 备注）
internal/directlist/       no-proxy 旁路（域名/IP → direct，**仅 L4 Route**，不开闸；私网段引擎内置；可选 `notes` 备注）
internal/proxygroups/      代理分组（Config{AutoCountry,ExcludeCountries,Groups,**Failover**}；Failover=每个 urltest 组的切换积极度,默认粘性:tolerance 150ms + **不中断已有连接**,见「登录到一半突然断连」那条坑）+ 国家解析(country.go：旗emoji/中英/国码→ISO)+**假节点指纹**(`junk.go`：跳转域名/请勿连接/Traffic Reset/Expire/剩余流量/套餐到期/配额 `GB|GB`/占位 server `123.123.213.213`)；injectOutbounds 前 `FilterEligibleNodes` 踢 junk+手动 disable。建 Auto(urltest)+**🌏 Overseas**+按国家+用户组，proxy=selector(default Auto)。Overseas 排除未识别国别(信息行进不了 Overseas，以前仍进 Auto——现由 junk 一并挡)。ExcludeCountries 默认 HK/MO/CN。sing-box 只有 selector/urltest,无 load-balance
internal/nodeoverride/     按 outbound tag 持久禁用（data/nodeoverrides.json）；刷新订阅不丢；`/api/node-overrides` + `POST /api/nodes/disable|enable` + `proxies nodes|disable|enable` + Proxies 页开关
internal/proxyscore/       **节点打分**（真实流量,非 generate_204 探测）：`score = (w_rel×可靠性 + w_lat×延迟 + w_tp×吞吐)/Σw`,观测存 data/proxyscores.json、超 `stale_hours`(默认6h) 的条目加载时丢弃(换订阅/换网络后旧数字描述的是另一条链路)。三条支柱各对应一次真实事故：① **预热**——攒满 `min_samples`(默认10) 真实样本前恒为中性 100，冷启动一次倒霉拨号不该把全组降级；② **连击而非均值**——第 N 次连续失败扣 N 倍(封顶 `max_streak`)，均值会忘记「它已经死了十分钟」，连击不会而且可解释(UI 直接印连击数)；③ **断路器(failsafe-go)与分数正交且绝不排除**——分数答「它一直以来有多好」(要 10 个样本)，断路器答「它现在还能不能用」(**从第 1 个样本就工作**,这才是死节点没法顶着预热 100 被选中的原因)。④ **黑洞判定**——握手通、发出去了、回包恒 0 的成员直接 0 分（`blackhole_streak` 默认 3，-1=关），**且压过预热**：假节点最能害人的正是它的头十条连接。⑤ **长流卡死（stream stall）**——打分只影响*新*拨号，挂死的长连接（大上传后下行静默，如 Cursor Agent）要靠 `wrapStreamStall` 中途掐断并 `RecordStreamStall` 降级（默认静默 20s / 武装 64KiB / 最短存活 8s；`stream_stall_sec=-1` 关）。⑥ **三层降级正交、各有恢复路径**——(a) 断路器 open 默认 delay 30s，之后 failsafe 仍标 Open 直到下一次 `TryAcquirePermit`（`Observe`/`NoteProbe`）；API 在 rem=0 时表面 half-open，避免读成永久封禁；(b) 黑洞 0 分只靠 `Download>0` 或 `NoteProbe` 清；**(b2) 连败降级靠成功探测平反到中性 50**（见下 ⑦）；(c) urltest `failedUntil` 冷却 5min 只挡 **Select 排名**（generate_204 可通而真 TLS 仍挂），**冷却中仍探测**并 `NoteProbe`——以前 `continue` 跳过探测 ⇒ 断路器/黑洞鸡生蛋永不恢复。拨号成功*不能*清黑洞。断路器 open **只降到最后备选**，见下「只降级不硬排除」。⑦ **成功探测能平反、但只到中性 50**——被连败打挂的成员分数垫底 ⇒ `Select` 不给流量 ⇒ 唯一能清 `FailStreak` 的 `Observe(true)` 永不发生，**棘轮**：一次全网闪断（睡眠/Wi-Fi 掉线，所有成员同时失败）就永久报废整个订阅，真机实测 39 个成员里 32 个停在 23–29 分、`fail_streak` 29–31，而探测每 30 秒都回 94–318ms，同一条记录里 `last_ok: true` 和 `reliability: 0` 并存；手动 reset 后吞吐 610 KB/s → 2.5 MB/s。所以成功探测把可靠性抬到 `neutralReliability`(50)、并让连败**衰减一档**（不是清零）—— 抬可靠性是**抬地板不是赋值**（健康成员的 92 不会被改成 50），连败衰减而非清零是因为它驱动的是惩罚**递增**，而「generate_204 能过、真 TLS 仍挂」是真实形状（`failedUntil` 冷却为它存在），清零会让这种成员永远只吃 1 倍惩罚；衰减则让真恢复的成员几轮走回 0、而持续失败的成员跑得比衰减快。**这两件事第一版被我绑在同一个 `if` 里**，于是真机部署后立刻看到 `rel=92` 且 `streak=1` 的成员永远甩不掉那个连败 —— 可靠性高低和「连续段是否结束」是两个问题。分数在第一次探测后就恢复（可靠性占 50% 权重），成员重新可选之后由真实流量说话 —— **只到中性**，因为探测是弱证据（generate_204 能过而真 TLS 仍挂，那正是 `failedUntil` 冷却存在的理由），平反后的成员排在有真实流量证明的成员之后，下一次真实失败从 50 而不是 100 往下掉。失败探测仍然**不** `Observe(false)`：探测能赦免、永不定罪。⑧ **只给真成员打分**——连接是在*开始*时被 track 的，`detector.outStr` 靠 `Now()` 跟到成员，而刚重建的 urltest 组还没选出成员、`Now()` 为空 ⇒ **组 tag 原样泄漏进打分库**。真机实测：国家组 `🇺🇸 US`/`🇯🇵 JP` 有打分记录且 `blackhole_streak` 已到 2 和 1，再来两条就会把**组自己**判死（0 分 + 强制开断路器），而钉死规则打的正是这个组（Claude→US）。`rebuild()` 存一份 `FilterEligibleNodes` 之后的成员快照（**原子快照不是 `m.mu`**——finalize sink 跑在 detect 引擎锁下，在那里回头拿 Manager 锁就是在造锁环），四个入口（`RecordEvent`/`RecordTransfer`/`NoteProbe`/`RecordStreamStall`）一律丢弃非成员；样本是**丢掉而不是改记到成员头上**（track 时组确实还没选，成员在那一刻不可知，要捞回来得在 finalize 拿拨号链）。⑨ **吞吐取窗口内最大值，不是中位数**——`bytes/墙上时钟`只有在「传输本身就是耗时原因」时才是吞吐；流式回答/SSE/长轮询按应用节奏传几百 KiB 耗几分钟，同样的算式把健康链路算成个位数 KB/s。真机实测每个成员的中位数都在 1.7–94 KB/s 而实测链路 2500 KB/s ⇒ 吞吐项对健康和不健康的成员**一样是 ~5 分**，20% 权重不含任何信息，还把所有成员的上限压在 **~79 分**（回归测试里用中位数正好复现 80.01）。**不对称性**是取最大值的理由：空闲只能把样本往**下**偏，而没有东西能传得比链路快，所以最大观测值是被动测量能给出的最接近「容量」的数；高的离群值被真实速率封顶，低的那侧毫无下限。「它现在好不好」由可靠性/延迟/断路器回答。延迟仍取中位数——那里离群值就是离群值。`View` 报告的统计量与打分用的**同一个**，否则徽章和它背后的数字会互相打脸。⑩ **机场信息行不进打分表**——`35.77 GB | 300 GB` / `Expire Date:` / `Traffic Reset:` 这类配额文本被 `FilterEligibleNodes` 挡在组外、永远拨不通，但打分列表是从**裸节点列表**建的，于是三条都以「100、preferred」坐在表里，和最好的真实出口无从区分；`MemberTags()` 现在也过同一个过滤器（**手动 disable 的节点保留**——那是真节点，历史通常正是禁用它的理由）。配置走 `proxygroups.Config.Scoring`(同一主题+复用 profile/posture 快照管道)，观测是观测、绝不被 profile 快照带走。本包**不 import sing-box**，桥接在 `internal/gateway/scorer.go` + `streamstall.go`
internal/policymigrate/    Permit⊥Route 一次性迁移（allow-*→permit+route-*；directlist 拷入 whitelist；profiles v2）
internal/customrules/      有序策略规则（Permit 与/或 Egress 正交；L4 有序；`MoveTop` 置顶非 pin；node self-heal）+ **策略包**（`presets.go`：Claude/OpenAI/Cursor/…；**AI 四包 `exit=pinned`，各钉死一个国家组**（Claude/Cursor/AI (other)=`🇺🇸 US`，OpenAI=`🇯🇵 JP`）+ 域名后缀 + **keyword 兜底**；**Cursor=`mixed`**：`api5.cursor.sh`→direct（Agent 长流稳），其余钉 US，且更具体规则须排在前；**China (wide)=permit** + **China-direct=route-direct** 拆开；共享 tag 时 Merge/Subtract）
internal/inboundcfg/       代理入站的**监听点**（data/inbound.json：listen/port，零值=沿用 base config 的 127.0.0.1:21584，故老机器逐字节不变）。**凭据不在这里**——由 `internal/users` 派生（`ProxyCredentials()`，applyMode 注入 mixed users），控制台没有独立的「入站账号」编辑器。两者不合并是刻意的：合并后每次改密码都得重新带上 listen/port，漏带就是静默清零（「POST 静默丢字段」那条坑的形状）。改端口会把所有客户端踢下线,故与 mode 同级带**死亡开关**(`--guard`,默认 60s);**不进 profile/posture 快照**(它是机器设施不是策略,激活一个上周的档不该换端口)。API 层另有一条 store 到不了的校验：**监听非 loopback 且没有任何账号设了代理密码 ⇒ 400**（那会在局域网上开一个匿名代理）
internal/retentioncfg/     日志与历史的轮转/保留（data/retention.json，两侧都是 lumberjack 参数）。**必须能在运行时换**，否则这轮只是把 8 个 serve flag 搬了个位置——装成系统服务后 flag 固化进 plist/unit，想调只能重装。lumberjack 没有 setter 且 `Write` 持它自己的锁 ⇒ 只能**换指针**：history 侧持 `s.mu` 关旧建新；logging 侧加一层 `rotator`（`io.Writer`，内含 `*lumberjack.Logger`）当 diode ring 的 sink，`SetRotation` 只替换里面那个。**拿「重跑一次 `Setup`」去换参数是错的**——它会把 fd 1/2 的重定向拆了重搭，那条路径上 sing-box 自己的日志会静默丢失
internal/modecfg/          捕获模式（manual/system/tun）的持久化。**判例：一个只存在于 launchd plist 参数里的设置，重启即失效，而裸重装——文档里的升级路径——会把它重写掉**；`internal/service/launchd.go` 的 `serveFlags()` 注释记着 `--mode` 为什么被从那里删掉。推论：**新配置进 store，不进服务定义**
internal/tuncfg/           TUN 高级选项（stack/mtu/strict_route/**auto_redirect**/address/exclude·include_package，applyMode 用；Linux 默认 auto_redirect 捕获 Docker/containerd bridge 出网；TUN 地址默认 `198.18.0.1/30` 避开 172.16/12）。**`Defaults()` 与 `Resolved()` 是两个问题**：`Defaults()` 播种文件——address 必须留空，否则今天这个 /30 会被冻进每台机器的 store，日后改常量只对新机器生效；`Resolved()` 描述行为（address = `198.18.0.1/30`），`/api/defaults` 用的是它。曾经回的是存储形态，于是控制台把这一项的默认渲染成「无」——而「无」正是 TUN 地址唯一不能取的值。**桌面 OS（非 Linux）TUN 必带 `route_exclude_address=privateCIDRs`**：Darwin 的 catch-all 是 `128.0/1`+`0/1`…，盖住整个 RFC1918；正常靠 en0 上更具体的 on-link 路由保 LAN，一旦那条路由闪断，内网（ssh dawn 等）会掉进隧道走 `direct` 再 5s 超时——真机四连 `direct/direct ms=5000 u=0`。Linux **不能**同样排除，否则 Docker/CNI 的 10/8·172.16/12 正好被 auto_redirect 要抓的流量被静默放跑
...
| **`e2e-tun-dind`** | **`DOCKER-USER` 链**：容器里再起一个**真 dockerd**，`docker run busybox` 出网；断言 sing-tun 往 `DOCKER-USER` 插的两条兼容规则（`iifname`/`oifname`，注释前缀 `!<table>: Docker compatibility `）在、容器被捕获、默认拒绝拦得住、Permit 后通，最后 `docker network create` **重写那条链之后规则被 monitor 调解回来且容器没断网**（CI 实测：install 后链上是 `tun0`，一次 `acl add permit` 触发 box 重建换成 `tun1`，`docker network create` 之后 monitor 按**新接口名**重插——比「规则还在」这条断言更强） | 上一层的 netns+bridge 与 docker0 是同一条内核路径，但**不会产生 Docker 自己的防火墙**。dockerd 建的 `DOCKER-USER` 规则跑在它自己的 accept **之前**，没有兼容规则时**容器什么都连不上而网关坚称全部捕获**。`redirect_nftables_docker.go` 整个文件（含 nftables monitor 调解）在此之前零覆盖——**那条链不存在，除非有真守护进程建它**。嵌套守护进程慢且脆，故不进每次 PR |
...
**日志（`internal/logging`）**：`zerolog` → `diode` → `lumberjack`，全部用现成库，不手搓。轮转参数：`--log-max-size`(MB,默认 32)、`--log-keep`(默认 3)、`--log-max-age`(天,0=只按个数)、`--log-compress`(默认开)；`--log-max-size 0` 关闭轮转。**这 8 个 flag（日志 4 + 历史 4）现在只是 `internal/retentioncfg` 的初值**：跟 `--auto-block` 同形，只有被真正敲过（`Flags().Changed()`）才覆盖 store，运行时改走 `retention set` / Settings 弹窗。装成系统服务后 flag 会固化进 plist/unit，那时它们再也改不动了——所以真正的事实源必须是 store。
...
| 服务 | 地址 |
|---|---|
| 代理入站 (mixed socks/http) | `127.0.0.1:21584`（**可改**：`internal/inboundcfg` / `inbound set --listen --port`，带死亡开关；不再需要手改 `config.json`）|
...
- **代理服务端（出口节点）**：`trust-proxy proxy run -c server.json`；一键生成：`trust-proxy proxy gen --type <ss|vless-reality|vless|vmess|trojan|anytls|hysteria2|tuic> --server <ip> --port <p>` → 输出服务端配置 + 客户端节点（Clash dict，可直接粘进 console）。TLS 协议自动内联自签证书（客户端 skip-cert-verify），vless-reality 免证书自动生成密钥对。
  **TUN 全流量**是它的一个模式而不是另一种部署：`install --mode tun` 开机就进，或运行时 `mode set tun`／控制台开关（都带死亡开关，不 confirm 就自动回滚）。`tun` 入站 + `auto_route`（Linux 另默认 `auto_redirect`/nftables）网络层接管**所有**出入网流量——木马的裸 socket 也逃不掉；**Linux 宿主机上 Docker/containerd bridge 容器出网**也进同一套 Permit/检测（macOS 上 Docker Desktop 在 VM 里，宿主机 TUN 管不到）。与其他 TUN 工具（Surge 增强模式等）互斥，用于**专用网关机/软路由**。检测与策略逻辑不变（同一 route）。构建需 `with_gvisor`（已在默认 TAGS）。
...
- **自动封禁独立于 posture（✅）** 以前写进当前槽的 deny 列表，切 Strict/Split 或激活 Profile 会被 `alignLiveStores` 整份替换而静默消失。现在进 `internal/quarantine`（独立文件 + L1 地板注入），`trust-proxy quarantine ls|release|permit` 与 Detection / Policy→Deny 页可查；**一键许可** = 解禁并写入 Permit（`POST /api/quarantine/permit`），只 release 在 Strict 下仍会被默认拒绝拦住、看起来像没解禁。顶栏有隔离横幅（`/api/status.quarantine` 计数），避免「TCP 连通后立刻 EOF」被当成远端挂了。
- **坑：封禁了但列表是空的（已修）** 现象正是自动封禁最坏的失败形态：**连接被杀了，隔离列表却是空的**——UI 上看不见、CLI 查不到、于是既无法跟踪也无法解禁/加白，只剩一个「连上就 EOF」的目的地。根因在 `detector` → `quarantine` 的交接：`banEvent` 把 `hostOnly(ev.Destination)` 当作 **ip** 参数传下去，而 **proxy/socks 模式下 sing-box 是按域名拨号的**，`Destination` 就是 `example.com:443`——传下去的「ip」其实是个主机名。`Store.Add` 里 `normalizeIP` 因此报错并**在循环之前就 return**，连同一次调用里那个**合法的 domain 一起丢掉**，什么都没落盘、什么都没注入。修在两层（按「回那一层改」）：① `banEvent` 先判断 `Destination` 到底是不是地址，不是就当域名走 domain 轴；② `Store.Add` **绝不让一个坏值带走一个好值**——记下合法的那半、把被拒的那半作为 error 报告（连接**已经**被杀了，整条拒绝等于留下一个不可达又不可见的目的地），`serve` 的 sink 相应改为「部分成功也要 apply」。**为什么单测没抓到**：喂进去的都是干净的 IP，而域名拨号在 socks 模式下是常态不是边角。回归：`internal/detect/ban_test.go`（5 种 host/dest 组合）、`internal/quarantine/store_hostname_test.go`、以及 `selftest` 的 `== quarantine ==`（记录 + 强制 + 解禁恢复三件事都断言，去掉修复会 FAIL 2 项）。真机复现路径：本地起一个只回几个字节的上传 sink（保证 upload/download 不对称满足 exfil「形状」判据）、`exfil_upload_bytes` 调到 1MiB、**按域名**经 :21584 上传 4MiB——修复前日志是 `quarantine add error="invalid IP \"…\"" ip=<域名>` 且 `/api/quarantine` 为 `[]`，修复后条目在 API/磁盘/顶栏计数三处都在，`POST /api/quarantine/permit` 一键解禁后流量恢复 200。
- **坑：登录到一半突然断连 = 组重新选举掐掉了活连接（已修）** 症状是「访问谷歌等社交媒体登录时随机断」，看起来像节点不稳定，实际是**我们自己的配置**：`buildProxyGroups` 给每个 urltest 组硬编码了 `interrupt_exist_connections: true`，而 sing-box 在选中成员变化时会调 `interruptGroup.Interrupt()` **主动 Close 所有已建立连接**（`protocol/group/urltest.go` 的 `performUpdateCheck`）。再叠加 sing-box 的 `tolerance` 默认只有 **50ms**（不是 0，`NewURLTestGroup` 里补的），跨境延迟抖动几十毫秒就足以让两个同样好的节点来回翻转——于是每 30s 一次探测就是一次「可能掐断你正在做的事」。**真机对照**（`/tmp` 起两个 HTTP CONNECT 出口 + 一个慢速滴数据的 origin，模拟被握住的登录会话）：`interrupt=true` 时重新选举瞬间 `curl` 报 `EXIT=56 Connection reset by peer`、只收到 tick 0；`interrupt=false` 时同一次重新选举照常发生（Auto 从 NODE-B 切到 NODE-A），会话 120 tick 全收完 `EXIT=0`。**修法不是换个更好的常量，而是把旋钮变成配置**（呼应「所有配置都应能在 settings 里设」）：新增 `proxygroups.Failover`（`probe_interval_seconds` / `tolerance_ms` / `idle_timeout_seconds` / `interrupt_existing_connections`），默认改为**粘性**——tolerance 150ms、**interrupt 关闭**。关掉 interrupt 不会拖慢故障切换：真正拨号/IO 失败走的是另一条路（`markFailed`/`wrapFailoverConn` 立即换节点），周期探测只负责「谁更快」这件不紧急的事。全链路落地：store（校验 + 加载时坏值回落默认，不 brick）→ `/api/proxygroups` + profile/posture 快照（**必须带上这个字段**，否则激活一个旧快照会把用户关掉的 interrupt 又打开）→ SDK `ProxyGroupsConfig/SetProxyGroupsConfig` → CLI `trust-proxy groups failover [--tolerance/--interrupt/…]`（无参数即查看，会把「未设置」显示成实际生效的默认值）→ Proxies 页「切换行为」卡片（每个旋钮一句人话解释，开 interrupt 有琥珀警告）。回归：`internal/proxygroups/failover_test.go`、`gateway_test.go` 的 `TestProxyGroups_FailoverTuningReachesEveryURLTest`（**每一个** urltest 组都要吃到配置——只修 Auto 会让断连变得更罕见因而更像随机）。
- **代理打分（✅，`internal/proxyscore`）** 上一条把切换变**粘**了，但粘性只让切换更少、没让它更**准**——urltest 选节点始终只看一个数字：`generate_204` 探测回来的延迟。**延迟不等于可用**（fork 里的 `failedUntil`/`markFailed`/`wrapFailoverConn` 就是为此手写的补丁，但它是二值的五分钟黑名单：记得「坏」，记不住「多坏 / 坏了几次 / 最近有没有好转」），而且**观测来源是探测不是真实流量**（对 generate_204 很快、对真正目标频繁重置的节点，探测看不见）。现在按真实流量打分：`score = (50×可靠性 + 30×延迟 + 20×吞吐)/100`，**惰性**（不新增后台探测）、**不迁移已有连接**（只影响后续新连接选谁，与上一条同一主张）。样本三处来源：拨号成功/失败+拨号延迟（fork `DialContext`/`ListenPacket` 成功分支 + `markFailed`）、早期 IO/TLS 失败（`markUnhealthy`）、字节/时长/吞吐（**白捡**——`detect` 引擎的 finalize sink 里 `Event.Outbound` 经 `detector.outStr` 已解析到**真实成员**而非组名，包一层即可，数据面一行不动）。fork 侧新增 `adapter/scorer.go` 的 `OutboundScorer`（与 `ConnectionTracker`/`DNSQueryTracker` 对称，`service.FromContext` 取；**nil ⇒ 行为与上游逐字节相同**，这是回归保险），`Select`/`pickForDial` 先按分数桶排序、**同桶内按延迟**（用户的「相同分数选择延迟最低的」，沿用现有 `tolerance`）。
  **铁律**（改这块前先读）：⓪ **降级的每一条路都必须有一条恢复路**——缺一条就是棘轮，而棘轮的症状是「换个机场也没用」，没人会怀疑打分；见上 ⑦。① **断路器 open 只降级、绝不硬排除**——`clearFailures` 的注释记着一次全组进冷却导致**整个网关没有出口**的事故，硬排除会把那个形状重新引进来；`Select`/`pickForDial` 的兜底层层退让（优先断路器闭合的 → 任意成员 → 连冷却中的也认），宁可拨一个坏节点也不返回 nil。② **预热不是可选的**——`min_samples`(10) 前恒 100，否则网络切换那一刻所有节点同时失败、同时被降级。③ **观测≠配置**——分数进 `data/proxyscores.json` 自己拥有，配置进 `proxygroups.Config.Scoring` 走 profile/posture 快照；把观测塞进快照，激活一个旧档就会拿昨天的数字描述今天的链路。
  全链路：store（校验 + 坏值回落默认）→ `/api/proxy-scores`(+`/reset`，响应里**带上当前权重与公式字符串**，让「为什么是这个分」在一个响应里读完) + `/api/proxygroups` 的 `scoring` 与三处快照 → SDK `ProxyScores()` → CLI `proxies scores|scores-reset`、`groups scoring` → Proxies 页分数徽章(hover 出分项/连击/断路器) + 明细表 + **「高级」折叠区的权重编辑**，公式与每项当前实测值就画在旁边（配置可调但不神秘）。**UI 绝不自己推导默认值和公式**——两份就会漂移，一律来自 `/api/proxy-scores`。预热中显示 `100 · 4/10` 而非光秃秃的 `100`：后者读起来是「测过了，很好」，恰是它的反面。
- **黑洞节点：订阅里的假代理（✅）** 用户报的形状是「很多流量都走到它们，就根本不通」。机场订阅里混着一类成员：**握手完全正常，但什么都不转发**。数据面每一个信号都说它健康——拨号**确实**成功了——于是 urltest 一直把流量交给它，而打分的三项对它也全是满分（可靠性看拨号、延迟看拨号、吞吐样本压根不产生）。判据只能来自**已完成的连接**：`Upload>0 && Download==0 && 握手成功`。三个条件各挡一类误判——要求 `Upload>0` 排除掉「用户自己取消了」（取消发生在发出去之前）；要求握手成功把「根本没连上那个节点」摘出去（那已经是拨号失败，在拨号那条路上算过分了）；连续 `blackhole_streak`(默认 3) 次才判，一次偶发的空响应不算。处置按用户选的：**可靠性归零 + 分数 0 + 强制打开断路器**，但**仍留在组里**（铁律①：硬排除会重演「全组无出口」），且**不杀在飞的连接**（与「登录到一半断连」同一主张）。**分数 0 压过预热**是这条的关键：预热存在的理由是「别凭一次倒霉就定罪」，而黑洞的证据是结论性的，恰恰是新订阅刚导入的头十条连接最需要它。收到任意一个回包立即清零恢复——机场修好一个节点是常态。全链路 `blackhole_streak` 一路到 `groups scoring --blackhole-streak` 与 Proxies 页高级区；节点行显示 **`黑洞 · N 条不通`** 而不是一个很低的分数（后者读起来像「测过了，比较慢」，是它的反面）。
  **这一轮修的两个 bug，第二个只有真机才暴露**：① **tag key 分裂**——`normalizeTag`。拨号侧上报的是裸 tag（`🇭🇰 Hong Kong丨02`），finalize sink 走 `detect.Event.Outbound`，而 `detector.outStr` 渲染成 `类型/tag`（`anytls/🇭🇰 Hong Kong丨02`）给连接页看。两个拼法 ⇒ 每个节点裂成两条记录：拨号那条永远收不到吞吐样本（吞吐项固定停在中性 50），传输那条永远 0 样本出不了预热。**两半各自看都合理**，所以单测全绿，只有真网关上的重复行才看得出来。按 `/` 前缀是不是**已知 outbound 类型**来剥（allow-list，不是「剥掉第一个斜杠前的东西」——节点可以叫 `airport/tokyo-01`）。② **`Handshook` 不能从 `ConnectMs > 0` 推**——相位耗时是**截断到毫秒**的，亚毫秒握手报 0，于是每一条快连接都会被读成「我们从没连上这个节点」，正好是黑洞判据赖以成立的那条区分的反面。改成从 TimingSource 的**时间戳存在与否**求出一个真的 `Event.Connected` 布尔量传下去。
  回归：`internal/proxyscore/store_test.go` 五条（含「取消的请求不算证据」「没握手不算证据」）、`internal/detect/engine_test.go` 的 `TestConnectedIsNotInferredFromTruncatedMilliseconds`(200µs 握手 ⇒ `connect==0` 但 `connected==true`)、`internal/api/proxygroups_wire_test.go`（**反射**跑 wire 往返 + 字段数对齐，手写字段清单本身就是会陈旧的那个东西）、以及 `selftest` 的 `== blackhole nodes ==`（假上游：接受 CONNECT、吞掉载荷、一个字节都不回；断言 0 分+断路器开+仍在组里+下一条连接换节点，去掉修复第一条即 FAIL）。**selftest 以前根本没接 finalize sink**（只有 `serve.go` 有），所以这个场景先要把那条线接上——现在两边都走 `Manager.RecordEvent`，映射只有一份。
- **AI 出口钉死国家，不再走 Overseas（✅）** 内置 AI 包原本全指向共享的 `🌏 Overseas` urltest，而那个组的成员是「所有非 HK/MO/CN 且能识别国别的节点」——真机上是 **26 个节点、14 个国家**，含 TR/VN/PH/TH/MY；urltest 按延迟排，从国内看这些恰恰经常最快。后果在 20 万条真实连接里看得很直白：**同一个登录态的 ChatGPT 会话一周内从 GB / KR / SG / VN 四个国家发出去**（`cdn.openai.com`→🇬🇧、`images.openai.com`→🇰🇷🇸🇬、`persistent.oaistatic.com`→🇻🇳）。对账号绑定的服务，**稳定的出口比快的出口重要**：跳国家换来的是重新登录、风控、或者干脆区域不支持。现在四个 AI 包 `exit=pinned`，各钉一个国家组（Claude/Cursor/AI (other)=`🇺🇸 US`，OpenAI=`🇯🇵 JP`）。**egress 用 `proxy`+`Node` 而不是 `node`**：没有该国节点时前者 self-heal 到默认 proxy 组，后者会被整条丢弃——而丢弃的规则**连 Permit 一起丢**，服务会变成默认拒绝而不是「换个出口」。同时补上真机跑出来的遗漏：`claudeusercontent.com`（Artifacts iframe）、`hcaptcha.com`（claude.ai 登录挑战）——这两个此前只靠运维手写的 keyword 规则兜住，内置包里根本没有；keyword 兜底（`anthropic`/`claude`/`openai`/`chatgpt`）也一并收编进包，代价写在 `Warning` 里（keyword 会为任何含该词的主机名开闸）。回归：`TestPresets_AIPacksPinExactlyOneCountry`（两个国家=同一个 bug，只是小一点）、`TestPresets_AIPacksCoverObservedHosts`（真机 host 清单，去掉 `claudeusercontent.com` 即 FAIL）。
- **网关常驻 CPU 的归因 + 两处真浪费（✅）** 现象是「网关 CPU 占用异常」。先量再改：`ps -o time=` 取**增量**（`%CPU` 是进程生命周期平均值，读它等于什么都没读），真机 **2.95% of one core、方差极小（2.24–3.29%）**，而同期吞吐在 8→70 KB/s 之间变了近十倍——**与流量不成正比就说明不是数据面**。归因靠本地起第二个实例做 A/B（`serve --data /tmp/... --api-addr :31585 --clash-addr :31586`，**两个 addr 必须一起给**：`injectClashSecret` 会用 `--clash-addr` 覆盖配置里的 `external_controller`，只改配置文件会撞上线上那个口）：空载 0 节点 manual = **0.02%**（我们自己那堆 ticker——netwatch 30s / history 30s / events 30s / scoreFlush 2min——合起来几乎免费）；灌 36 个假节点复刻线上组拓扑后 = **0.28%**；再灌 ~10 次解析/秒 = **1.08%**。剩下的 ~2% 只剩 TUN 数据面一个解释（无特权测不了，要 `sudo sample <pid>`）。**结论是没有跑飞的循环**，但过程里量出两处确凿的浪费：
  ① **探测风暴：一个节点被探 N 次，N = 它属于几个组。** 真机 36 个节点被 18 个组瓜分出 **103 个成员位**（Auto 36 + Overseas 26 + 16 个国家组），而 urltest 的去重条件是「delay history 比 interval 新就跳过」——这个条件**结构上不可能成立**：每个组的 ticker 都是同一个 box 建的、同一个周期，于是同时开火，全部在任何一次探测写回 history **之前**读它。本地复刻实测：一次 box 启动打出 **103 个探测连接、36 个节点各 3 次**。更糟的是失败路径——探测失败或成员在拨号冷却里会**删掉** history 条目，于是那条新鲜度判断从此永不命中，**最该少探的死节点/junk 节点反而每组每轮都探一遍**。修法在拥有这个判断的那一层（fork 的 urltest）：`HistoryStorage` 加 `ClaimProbe/ReleaseProbe`，按**尝试**而非**结果**去重，窗口=interval。HistoryStorage 是 **per-box**（组从 box 的 service context 取），所以一个进程里的多个 box（测试、selftest）不会互相吞掉探测。实测 **103 → 36，每节点正好 1 次**，且 18 个组全都仍有 selection + delay（跳过探测的组照样从共享 history 读排名）。注意 `force=true`（暂停恢复、手动 delay）仍绕过去重。
  ② **每条连接一个 goroutine + 1Hz ticker**（`wrapStreamStall`）。CPU 不是重点（一次 timer fire 很便宜），**生命周期**才是：watcher 只在下一个 tick 才知道连接结束了，而且**只认走 wrapper 的 `Close()`**——以别的方式结束的连接会把 goroutine 和那个 1Hz timer 留到进程结束，顺带钉住 Event 和 conn 不让回收。长连接是常态不是边角：真机上最老的那条代理连接 **9.8 小时**，也就是 ~35000 次「什么都没决定」的唤醒。改成**一个 sweeper**（空了就停、来新连接再起，所以空闲网关一个 timer 都不持有）。顺带修了一个**误判**：`Read` 返回错误（含 EOF）以前不会结束 watch，于是一条正常结束的大上传连接，在 `stallFor` 静默之后会被当成卡死——**把节点降级**。现在 read 出错即结束 watch。回归：`common/urltest/probe_test.go`（含「失败的探测也要关上窗口」「两个 storage 互不干扰」）、`streamstall_test.go` 的 sweeper 三条（用 `countingConn` 数 Close 次数来**区分 kill 与 drop**——光看 watch 集合大小两者都是消失，那样的断言没有牙齿，第一版就是这么写错的）。
...
- **Linux TUN 捕获容器出网（✅）** `auto_redirect` 默认开（仅 Linux 注入；旧 `tun.json` 缺字段时迁移为 true）；TUN 地址默认改 `198.18.0.1/30` 避开 Docker `172.16/12`；`tun get|set --auto-redirect/--address` + Settings。macOS Docker Desktop 仍不在捕获面（VM 隔离）。转发失败关闭（未进 TUN 则 drop）仍属 `docs/egress-enforcement-risks.md` 暂缓项。
...
- **里程碑 19（✅）Settings 面整体清理**。诉求是「**所有的配置，都应当能够在 settings 里设置，目前很多设置都不透明，并且不知道是干嘛的**」。审计发现配置散在 8 处而 Settings 页只有两张卡（TUN + 语言），其中最危险的四个轴（捕获模式/路由模式/安全姿态/自动处置）**只在顶栏**——顶栏放不下解释文字，于是最该解释的东西恰恰最不透明；检测阈值 Go 侧 28 个字段 TS 侧只画了 18 个；**代理入站端口只有 `config.json`**（而那份文件文档上写着不该手改）；**日志/历史保留 8 个参数只有 serve 启动参数**（装成系统服务后固化进 plist/unit，想调只能重装）。
  **IA 参考 Clash Verge**（照抄结构不照抄框架）：一个 `SettingRow` 原语凭有没有 `onClick`/`to` 在值行/动作行/跳转行之间切换，label 右侧 ⓘ/⚙/⚠ 图标自带语义，复杂域用弹窗承载并带「恢复默认」。**混合式**：Settings 吸收所有没有自己页面的配置，DNS 与代理打分保留专页（带实时上下文的列表编辑器，塞进弹窗会挤）、给一行 + chevron 跳过去；Detection 页此后只留观测。**每个旋钮都必须有一句人话**——那是诉求的另一半，比搬位置更重要，文案直接搬 Go 字段注释里的真机数字。
  **三条自己给自己定的规矩**：① **默认值只有一个来源：服务端**（新增 `GET /api/defaults` 聚合各域，UI 的「恢复默认」和「(默认 X)」都从它取；TS 里再抄一份就是两份会漂移的真相，`/api/proxy-scores` 连公式一起回给前端是先例）；② **一个设置只有一个写者**（顶栏与 Settings 挂**同一个**组件——`switchers.tsx`，不是两份逻辑；刚修掉的自动处置 bug 正是「两个写者只有一个持久化」）；③ **新配置进 store，不进服务定义**（见 `internal/modecfg`）。
...
- **里程碑 20（🟡）K8s DaemonSet 节点级网关**。诉求是「让 k8s 集群的服务都能有一个漂亮的全球加速能力」，而它和上一条（Linux TUN 走 nftables 全接管）**是同一件事**：K8s Pod 出网走 `veth → CNI bridge → forward 链`，与 Docker 容器出网是同一条内核路径。所以形态选 **DaemonSet（hostNetwork + NET_ADMIN + `/dev/net/tun`）而不是集中式出口 Service**——后者要求每个应用改配置指到一个 ClusterIP，而 DaemonSet 是**应用零改造**：Pod 照常出网，包在离开节点前被 `auto_redirect` 捞进隧道。这个主张不是断言，是 `e2e-tun` 证的。
  ...
  **第五个坑（只有 e2e 抓得到，且形状最坏——它让 preflight 反着来）**：`internal/doctor.DetectNftables` 报的 `usable` 是 DaemonSet/Helm 的 preflight initContainer 唯一的门。第一次跑 `TestLinuxTUNWithoutNftBinary` 时它**在 baseline 断言上就挂了**：在**同一个镜像**里 `TestLinuxTUNCapturesForwardedBridgeTraffic` 刚刚证明了 sing-box 表建起来了、转发流量被捕获、默认拒绝对它生效，而 `doctor nftables --json` 回的是 `{"supported": false, "has_nft_binary": true, "usable": false}`。根因是**探测问的不是它预测的那件事**：老实现要求 `nft` 在 PATH **且** `/proc/net/netfilter/nf_tables` 能 stat，而 sing-tun 走的是 netlink（`sagernet/nftables` → `mdlayher/netlink`，`redirect*.go` 里对 `nft` 一次 exec 都没有），启动前只跑 `nftables.New()` + `ListTablesOfFamily(TableFamilyIPv4)`（`initializeNFTables`）。于是 preflight 会**拒绝一台捕获确实工作的节点**,正是它想防的那件事的反面。修法是回到拥有这个判断的那一层：`Supported`/`Usable` 一律由**与 sing-tun 逐字相同的 netlink 探测**决定（`nftables_probe_linux.go`，非 Linux 有 stub），`HasNftBinary` 降级为一条**只报不拦**的可调试性事实（镜像里留 `nft` 是为了出事时能 `nft list ruleset`，不是为了捕获）。牙齿在两处：包内三条单测（探针可替换，断言「没有 nft 也 usable」「探针失败必须 usable=false 且诊断原样透出」「Supported 与 Usable 永不分家」），以及 e2e 里把原来那半个「观测」改成断言——**删掉 `nft` 二进制后 `usable` 不许翻转、捕获不许消失**，这条单测替不了（单测能 stub 探针，真容器不能）。
