...
internal/proxyscore/       **节点打分**（真实流量,非 generate_204 探测）：`score = (w_rel×可靠性 + w_lat×延迟 + w_tp×吞吐)/Σw`,观测存 data/proxyscores.json、超 `stale_hours`(默认6h) 的条目加载时丢弃(换订阅/换网络后旧数字描述的是另一条链路)。三条支柱各对应一次真实事故：① **预热**——攒满 `min_samples`(默认10) 真实样本前恒为中性 100，冷启动一次倒霉拨号不该把全组降级；② **连击而非均值**——第 N 次连续失败扣 N 倍(封顶 `max_streak`)，均值会忘记「它已经死了十分钟」，连击不会而且可解释(UI 直接印连击数)；③ **断路器(failsafe-go)与分数正交且绝不排除**——分数答「它一直以来有多好」(要 10 个样本)，断路器答「它现在还能不能用」(**从第 1 个样本就工作**,这才是死节点没法顶着预热 100 被选中的原因)。断路器 open **只降到最后备选**，见下「只降级不硬排除」。配置走 `proxygroups.Config.Scoring`(同一主题+复用 profile/posture 快照管道)，观测是观测、绝不被 profile 快照带走。本包**不 import sing-box**，桥接在 `internal/gateway/scorer.go`
