...
- **官方 dashboard（`webui/`）保持不改**，只做运行时监控（sing-box `service/api` :9095，Connect）。不改 → upstream 同步干净。
- **我们自建的 React 控制台（`console/`，React 19 + Vite + TS）** 承载管理+安全，由后端 `internal/api`（:9096）serve，**浏览器只连 :9096 单一 origin**。连接数据由后端**代理 Clash API**（浏览器不碰 Clash secret）。
- 首版已实现：**订阅/节点管理**（列表/添加/apply/刷新/删除）+ **实时连接**（列表/断开/全断，2s 轮询）。
- console 目前**从磁盘 `console/dist` serve**（`make console` 构建）；go:embed 单二进制后续再上。数据接口只走我们自己的 `/api/*`（不绑官方 Connect / Clash REST 的前端）。
