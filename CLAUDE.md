...
internal/detect/           检测引擎（事件环形缓冲 + 字节计数 + 威胁情报匹配 + 持久化恢复）；**阈值全部可配**（`ApplyConfig`，见 internal/detectcfg）；exfil 需「形状」而非只看字节（上传/下载比 或 首见目的地）；`SetDisposalReady` 让处置等 Permit 索引就绪
