...
internal/customrules/      有序策略规则（Permit 与/或 Egress 正交；L4 有序；`MoveTop` 置顶非 pin；node self-heal）+ **策略包**（`presets.go`：Claude/OpenAI/Cursor/…；**Cursor=`mixed`**：`api5.cursor.sh`→direct（Agent 长流稳），其余→Overseas，且更具体规则须排在前；**China (wide)=permit** + **China-direct=route-direct** 拆开；共享 tag 时 Merge/Subtract；Overseas 给地区受限服务）
