...
```
订阅抓取按 UA 放行（默认 `clash-verge/v2.0.0`，可 `--ua` 改）。**真实订阅请在本机（住宅 IP）测**——机房/风险 IP 常被机场拒发内容。
解析支持 sing-box JSON（无损直取）与 base64/明文 share 链（vless/trojan/ss/vmess/hysteria2/tuic）；Clash YAML 暂不支持。
