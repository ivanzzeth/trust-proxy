...
// Without a sniffed domain the detector's host is the destination, which carries
// a port. A live gateway with sniff off (plain socks, UDP) reported nothing at all
// because "cloudflare-dns.com:443" matched no endpoint in the list.
func TestDNSBypassMatchesUnsniffedHostWithPort(t *testing.T) {
	e, _, got := newTestEngine(t)
	e.ApplyConfig(apitypes.DetectionConfig{
		BeaconEnabled: false, DGAEnabled: false,
		DNSBypassDetect: true, DNSBypassReAlertSec: 3600,
	...
	e.Track("tcp", "cloudflare-dns.com:443", "cloudflare-dns.com:443", "127.0.0.1:1", "curl", "", "proxy")
	if n := countKind(*got, KindDNSBypass); n != 1 {
		t.Fatalf("%d findings for an unsniffed DoH connection, want 1", n)
	}
	// And the cooldown still keys off one endpoint, not the port-bearing form.
	e.Track("tcp", "cloudflare-dns.com:443", "cloudflare-dns.com:443", "127.0.0.1:1", "curl", "", "proxy")
	if n := countKind(*got, KindDNSBypass); n != 1 {
		t.Fatalf("%d findings, want the repeat suppressed", n)
	}
}
