...
func (s *Store) AddProcess(p string) (Rules, error) {
	...
	return s.mutate(func() { s.data.Processes = liststore.Add(s.data.Processes, p) })
}
func (s *Store) RemoveProcess(p string) (Rules, error) {
	return s.mutate(func() { s.data.Processes = liststore.Remove(s.data.Processes, p) })
}
func (s *Store) AddDevice(ip string) (Rules, error) {
	...
	return s.mutate(func() { s.data.Devices = liststore.Add(s.data.Devices, ip) })
}
func (s *Store) RemoveDevice(ip string) (Rules, error) {
	return s.mutate(func() { s.data.Devices = liststore.Remove(s.data.Devices, ip) })
}
