...
export default function RuleSets() {
  ...
  const add = useMutation({ mutationFn: api.addRuleSet, onSuccess: () => { toast.success('Imported'); invalidate(); }, onError: err });
  ...
  return (
    <div>
      <PageHeader title="Rule Sets" description="Import public sing-box rule sets (geosite/geoip/ad-block). Roles inject into the default-deny route." />
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <Card>
          <CardHeader className="pb-3"><CardTitle className="text-sm">Add by URL</CardTitle></CardHeader>
          <CardContent className="space-y-2">
            <Input placeholder="tag (unique)" value={tag} onChange={(e) => setTag(e.target.value)} />
            <Input placeholder="https://…/xxx.srs  (or .json)" value={url} onChange={(e) => setUrl(e.target.value)} />
            ...
            <p className="text-xs text-muted-foreground">block sits above the whitelist (reject); allow-* sit just above the catch-all.</p>
          </CardContent>
        </Card>
      </div>
          ...
          <Table>
            ...
            <TableBody>
              {(!sets || sets.sets.length === 0) && (
                <TableRow className="hover:bg-transparent">
                  <TableCell colSpan={5} className="py-10 text-center text-muted-foreground">No rule sets imported</TableCell>
                </TableRow>
              )}
              {(sets?.sets ?? []).map((rs) => (
                <TableRow key={rs.tag}>
